...
# Consulta SQL para obtener las 10 mejores ventas por artículo de 2024
query = """
    SELECT 
        Articulo,
        SUM(Unidades) AS TotalUnidades
    FROM (
        SELECT Articulo, Unidades FROM articulos_enero2024
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_febrero2024
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_marzo2024
        UNION ALL
        SELECT Articulos AS Articulo, Unidades FROM articulos_abril2024
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_mayo2024
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_junio2024
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_julio2024
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_agosto2024
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_septiembre2024
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_octubre2024
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_noviembre2024
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_diciembre2024
    ) AS Todo2024
    GROUP BY Articulo
...
fig.update_layout(
    title=dict(
        text="<b>Top 10 Artículos Más Vendidos en 2024</b>",
        x=0.5,
        xanchor='center',
        font=dict(
...
fig.write_html("dash/static/graficas/grafica_top_10_articulos_2024.html")

print("Gráfico de top 10 artículos guardado como 'dash/static/graficas/grafica_top_10_articulos_2024.html'")
