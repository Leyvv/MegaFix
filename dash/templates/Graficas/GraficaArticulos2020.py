...
# Consulta SQL para obtener las 10 mejores ventas por artículo de 2020
query = """
    SELECT 
        Articulo,
        SUM(Unidades) AS TotalUnidades
    FROM (
        SELECT Articulo, Unidades FROM articulos_enero2020
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_febrero2020
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_marzo2020
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_abril2020
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_mayo2020
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_junio2020
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_julio2020
        UNION ALL
        -- Agosto usa "Articulos"
        SELECT Articulos AS Articulo, Unidades FROM articulos_agosto2020
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_septiembre2020
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_octubre2020
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_noviembre2020
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_diciembre2020
    ) AS Todo2020
    GROUP BY Articulo
...
fig.update_layout(
    title=dict(
        text="<b>Top 10 Artículos Más Vendidos en 2020</b>",
        x=0.5,
        xanchor='center',
        font=dict(
...
fig.write_html("dash/static/graficas/grafica_top_10_articulos_2020.html")

print("Gráfico de top 10 artículos guardado como 'dash/static/graficas/grafica_top_10_articulos_2020.html'")
