...
# Consulta SQL para obtener las 10 mejores ventas por artículo de 2022
query = """
    SELECT 
        Articulo,
        SUM(Unidades) AS TotalUnidades
    FROM (
        SELECT Articulo, Unidades FROM articulos_enero2022
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_febrero2022
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_marzo2022
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_abril2022
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_mayo2022
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_junio2022
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_julio2022
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_agosto2022
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_septiembre2022
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_octubre2022
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_noviembre2022
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_diciembre2022
    ) AS Todo2022
    GROUP BY Articulo
...
fig.update_layout(
    title=dict(
        text="<b>Top 10 Artículos Más Vendidos en 2022</b>",
        x=0.5,
        xanchor='center',
        font=dict(
...
fig.write_html("dash/static/graficas/grafica_top_10_articulos_2022.html")

print("Gráfico de top 10 artículos guardado como 'dash/static/graficas/grafica_top_10_articulos_2022.html'")
