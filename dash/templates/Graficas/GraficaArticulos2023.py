...
# Consulta SQL para obtener las 10 mejores ventas por artículo de 2023
query = """
    SELECT 
        Articulo,
        SUM(Unidades) AS TotalUnidades
    FROM (
        SELECT Articulo, Unidades FROM articulos_enero2023
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_febrero2023
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_marzo2023
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_abril2023
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_mayo2023
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_junio2023
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_julio2023
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_agosto2023
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_septiembre2023
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_octubre2023
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_noviembre2023
        UNION ALL
        SELECT Articulo, Unidades FROM articulos_diciembre2023
    ) AS Todo2023
    GROUP BY Articulo
...
fig.update_layout(
    title=dict(
        text="<b>Top 10 Artículos Más Vendidos en 2023</b>",
        x=0.5,
        xanchor='center',
        font=dict(
...
fig.write_html("dash/static/graficas/grafica_top_10_articulos_2023.html")

print("Gráfico de top 10 artículos guardado como 'dash/static/graficas/grafica_top_10_articulos_2023.html'")
